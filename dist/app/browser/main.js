...
function standardEncoding(v3) {
  return encodeURIComponent(v3).replace(STANDARD_ENCODING_REGEX, (s11, t42) => STANDARD_ENCODING_REPLACEMENTS[t42] ?? s11);
}
...
function provideHttpClient(...features) {
  if (ngDevMode) {
    const featureKinds = new Set(features.map((f11) => f11.\u0275kind));
...
var HttpResourceImpl = class extends ResourceImpl {
  ...
  constructor(injector, request, defaultValue, debugName, parse, equal) {
    super(request, ({
      ...
      let resolve;
      const promise = new Promise((r85) => resolve = r85);
      const send = (value) => {
        stream.set(value);
        resolve?.(stream);
        resolve = void 0;
...
function getClosestRouteInjector(snapshot) {
  ...
  for (let s11 = snapshot.parent; s11; s11 = s11.parent) {
    const route = s11.routeConfig;
...
var Tree = class {
  ...
  pathFromRoot(t42) {
    return findPath(t42, this._root).map((s11) => s11.value);
  }
};
...
function matchWithChecks(segmentGroup, route, segments, injector, urlSerializer, abortSignal) {
  ...
  return runCanMatchGuards(injector, route, segments, urlSerializer, abortSignal).pipe(map((v3) => v3 === true ? result : __spreadValues({}, noMatch$1)));
}
...
var Recognizer$1 = class Recognizer {
  ...
  async match(rootSegmentGroup) {
    ...
    try {
      ...
      if (e59 instanceof AbsoluteRedirect$1) {
        this.urlTree = e59.urlTree;
        return this.match(e59.urlTree.root);
      }
      if (e59 instanceof NoMatch$1) {
  ...
  async processSegment(injector, routes2, segmentGroup, segments, outlet, allowRedirects, parentRoute) {
    for (const r85 of routes2) {
      try {
        ...
        if (e59 instanceof NoMatch$1 || isEmptyError(e59)) {
          continue;
        }
        throw e59;
      }
...
function recognize(injector, configLoader, rootComponentType, config, serializer, paramsInheritanceStrategy, abortSignal) {
  ...
  return mergeMap((t42) => of(t42).pipe(switchMap((t43) => recognizeImpl(injector, configLoader, rootComponentType, config, t43.extractedUrl, serializer, paramsInheritanceStrategy, abortSignal)), map(({
...
async function loadChildren(route, compiler, parentInjector, onLoadEndListener) {
  ...
  const t42 = await maybeResolveResources(maybeUnwrapDefaultExport(loaded));
  let factoryOrRoutes;
  if (t42 instanceof NgModuleFactory$1 || Array.isArray(t42)) {
    factoryOrRoutes = t42;
  } else {
    factoryOrRoutes = await compiler.compileModuleAsync(t42);
  }
...
var NavigationTransitions = class _NavigationTransitions {
  ...
  setupNavigations(router) {
    ...
    return this.transitions.pipe(filter((t42) => t42 !== null), switchMap((overallTransitionState) => {
      let completedOrAborted = false;
      const abortController = new AbortController();
      return of(overallTransitionState).pipe(switchMap((t42) => {
        ...
        if (this.urlHandlingStrategy.shouldProcessUrl(t42.rawUrl)) {
          return of(t42).pipe(switchMap((t43) => {
          ...
          }), recognize(this.environmentInjector, this.configLoader, this.rootComponentType, router.config, this.urlSerializer, this.paramsInheritanceStrategy, abortController.signal), tap((t43) => {
      ...
      }), checkGuards(this.environmentInjector, (evt) => this.events.next(evt)), tap((t42) => {
        ...
        const loaders = loadComponents(t42.targetSnapshot.root);
        return loaders.length === 0 ? of(t42) : from(Promise.all(loaders).then(() => t42));
...
var RouterLinkActive = class _RouterLinkActive {
  ...
  set routerLinkActive(data) {
    ...
    this.classes = classes.filter((c27) => !!c27);
  }
  ngOnChanges(changes) {
    this.update();
  }
...
var AsyncAnimationRendererFactory = class _AsyncAnimationRendererFactory {
  ...
  loadImpl() {
    const loadFn = () => this.moduleImpl ?? import("./chunk-ZNRMTAZV.js").then((m5) => m5);
...
var routes = [
  ...
  {
    path: "generate",
    loadComponent: () => import("./chunk-EH5V6GP2.js").then((m5) => m5.GeneratePageComponent)
  },
  {
    path: "interview",
    loadComponent: () => import("./chunk-WU6ZKZQL.js").then((m5) => m5.InterviewPageComponent)
  },
  {
    path: "**",
    redirectTo: "generate"
